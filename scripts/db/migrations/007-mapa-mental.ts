...
/**
 * Migração 007 — tipo de artefato `mindmap`.
 *
 * `artifacts.kind` tem um CHECK que lista os tipos aceitos, e mapa mental é o
 * quinto. Escrita como .ts, e não como par de .sql, porque os dois bancos
...
const TIPOS = "'markdown', 'code', 'svg', 'mermaid', 'mindmap'";
...
async function sqliteJaTem(run: SqlRunner): Promise<boolean> {
  ...
  return rows.some((row) => String(row.sql ?? '').includes('mindmap'));
}
...
export const migration: Migration = {
  ...
  up: async (run, ctx) => {
    ...
    if (ctx.driver === 'postgres') {
      ...
      log('artifacts.kind aceita mindmap.');
      return;
    }

    if (await sqliteJaTem(run)) {
      log('artifacts.kind já aceitava mindmap; nada a fazer.');
      return;
    }
    ...
    log(`artifacts reconstruída com ${Number(total)} registro(s); kind aceita mindmap.`);
  },
};
